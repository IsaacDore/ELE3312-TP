...
static ili9341_two_dimension_t
ili9341_screen_size(ili9341_screen_orientation_t orientation);
static uint8_t
ili9341_screen_rotation(ili9341_screen_orientation_t orientation);

static int32_t interp(int32_t x, int32_t x0, int32_t x1, int32_t y0,
                      int32_t y1);
ili9341_two_dimension_t
ili9341_clip_touch_coordinate(ili9341_two_dimension_t coord,
...
ili9341_t *ili9341_new(
    ...
    GPIO_TypeDef *reset_port, uint16_t reset_pin, GPIO_TypeDef *tft_select_port,
    uint16_t tft_select_pin, GPIO_TypeDef *data_command_port,
      ...
      if (itsSupported != touch_support ||
        ...
        if (NULL != (lcd = malloc(sizeof(ili9341_t)))) {
          ...
          if (touch_support) {
            ...
            lcd->touch_scalar =
                (ili9341_scalar_calibrator_t){{{0U}, {0U}}, {{0U}, {0U}}};
            lcd->touch_3point = (ili9341_3point_calibrator_t){
                {{0U}, {0U}}, 0, 0, 0.0F, 0.0F, 0.0F, 0.0F};
  ...
  case itpPressed:
    if ((itpNONE == pressed) || (itpNotPressed == pressed)) {
      ...
      if (NULL != lcd->touch_pressed_end) {
        ...
        lcd->touch_pressed_end(lcd, lcd->touch_coordinate.x,
                               lcd->touch_coordinate.y);
      }
    }
    break;
...
ili9341_touch_pressed_t ili9341_touch_pressed(ili9341_t *lcd) {
  ...
  if (__GPIO_PIN_CLR__ ==
      HAL_GPIO_ReadPin(lcd->touch_irq_port, lcd->touch_irq_pin)) {
    return itpPressed;
  } else {
...
ili9341_touch_coordinate(ili9341_t *lcd, uint16_t *x_pos, uint16_t *y_pos) {
  if (NULL == lcd) {
    return itpNONE;
  }
  ...
  MODIFY_REG(lcd->spi_hal->Instance->CR1, SPI_CR1_BR,
  ...
  while ((itpPressed == ili9341_touch_pressed(lcd)) && (sample--)) {
    ...
    HAL_SPI_Transmit(lcd->spi_hal, (uint8_t *)x_cmd, sizeof(x_cmd),
                     __SPI_MAX_DELAY__);
    HAL_SPI_TransmitReceive(lcd->spi_hal, (uint8_t *)x_cmd, x_raw,
                            sizeof(x_raw), __SPI_MAX_DELAY__);

    HAL_SPI_Transmit(lcd->spi_hal, (uint8_t *)y_cmd, sizeof(y_cmd),
                     __SPI_MAX_DELAY__);
    HAL_SPI_TransmitReceive(lcd->spi_hal, (uint8_t *)y_cmd, y_raw,
                            sizeof(y_raw), __SPI_MAX_DELAY__);
  ...
  HAL_SPI_Transmit(lcd->spi_hal, (uint8_t *)sleep, sizeof(sleep),
                   __SPI_MAX_DELAY__);
  ...
  ili9341_two_dimension_t coord = ili9341_project_touch_coordinate(
      lcd, x_avg / req_samples, y_avg / req_samples);
...
void ili9341_calibrate_scalar(ili9341_t *lcd, uint16_t min_x, uint16_t min_y,
...
void ili9341_calibrate_3point(ili9341_t *lcd, uint16_t scale_width,
                              uint16_t scale_height, int32_t screen_a_x,
                              int32_t screen_a_y, int32_t screen_b_x,
                              int32_t screen_b_y, int32_t screen_c_x,
                              int32_t screen_c_y, int32_t touch_a_x,
                              int32_t touch_a_y, int32_t touch_b_x,
                              int32_t touch_b_y, int32_t touch_c_x,
                              int32_t touch_c_y) {
  if (NULL == lcd) {
    return;
  }
  ...
  lcd->touch_3point.scale =
      (ili9341_two_dimension_t){{scale_width}, {scale_height}};

  int32_t delta = ((touch_a_x - touch_c_x) * (touch_b_y - touch_c_y)) -
...
void ili9341_spi_tft_select(ili9341_t *lcd) {
  ...
  HAL_GPIO_WritePin(lcd->tft_select_port, lcd->tft_select_pin,
                    __GPIO_PIN_CLR__);
}
...
void ili9341_spi_touch_select(ili9341_t *lcd) {
  ...
  HAL_GPIO_WritePin(lcd->touch_select_port, lcd->touch_select_pin,
                    __GPIO_PIN_CLR__);
}
...
void ili9341_spi_slave_select(ili9341_t *lcd, ili9341_spi_slave_t spi_slave) {
  switch (spi_slave) {
  case issDisplayTFT:
...
void ili9341_spi_slave_release(ili9341_t *lcd, ili9341_spi_slave_t spi_slave) {
  switch (spi_slave) {
...
void ili9341_spi_write_command(ili9341_t *lcd, ili9341_spi_slave_t spi_slave,
                               uint8_t command) {
...
void ili9341_spi_write_data(ili9341_t *lcd, ili9341_spi_slave_t spi_slave,
                            uint16_t data_sz, uint8_t data[]) {
  __SLAVE_SELECT(lcd, spi_slave);
...
void ili9341_spi_write_data_read(ili9341_t *lcd, ili9341_spi_slave_t spi_slave,
                                 uint16_t data_sz, uint8_t tx_data[],
                                 uint8_t rx_data[]) {
  __SLAVE_SELECT(lcd, spi_slave);

  HAL_GPIO_WritePin(lcd->data_command_port, lcd->data_command_pin,
                    __GPIO_PIN_SET__);
  HAL_SPI_TransmitReceive(lcd->spi_hal, tx_data, rx_data, data_sz,
                          __SPI_MAX_DELAY__);
...
void ili9341_spi_write_command_data(ili9341_t *lcd,
                                    ...
                                    uint8_t command, uint16_t data_sz,
                                    uint8_t data[]) {
  __SLAVE_SELECT(lcd, spi_slave);
...
static void ili9341_initialize(ili9341_t *lcd) {
  ...
  // POWER CONTROL A
  ili9341_spi_write_command_data(lcd, issNONE, 0xCB, 5,
                                 (uint8_t[]){0x39, 0x2C, 0x00, 0x34, 0x02});

  // POWER CONTROL B
  ili9341_spi_write_command_data(lcd, issNONE, 0xCF, 3,
                                 (uint8_t[]){0x00, 0xC1, 0x30});

  // DRIVER TIMING CONTROL A
  ili9341_spi_write_command_data(lcd, issNONE, 0xE8, 3,
                                 (uint8_t[]){0x85, 0x00, 0x78});

  // DRIVER TIMING CONTROL B
  ili9341_spi_write_command_data(lcd, issNONE, 0xEA, 2,
                                 (uint8_t[]){0x00, 0x00});

  // POWER ON SEQUENCE CONTROL
  ili9341_spi_write_command_data(lcd, issNONE, 0xED, 4,
                                 (uint8_t[]){0x64, 0x03, 0x12, 0x81});

  // PUMP RATIO CONTROL
  ili9341_spi_write_command_data(lcd, issNONE, 0xF7, 1, (uint8_t[]){0x20});

  // POWER CONTROL,VRH[5:0]
  ili9341_spi_write_command_data(lcd, issNONE, 0xC0, 1, (uint8_t[]){0x23});

  // POWER CONTROL,SAP[2:0];BT[3:0]
  ili9341_spi_write_command_data(lcd, issNONE, 0xC1, 1, (uint8_t[]){0x10});

  // VCM CONTROL
  ili9341_spi_write_command_data(lcd, issNONE, 0xC5, 2,
                                 (uint8_t[]){0x3E, 0x28});

  // VCM CONTROL 2
  ili9341_spi_write_command_data(lcd, issNONE, 0xC7, 1, (uint8_t[]){0x86});

  // MEMORY ACCESS CONTROL
  ili9341_spi_write_command_data(lcd, issNONE, 0x36, 1, (uint8_t[]){0x48});

  // PIXEL FORMAT
  ili9341_spi_write_command_data(lcd, issNONE, 0x3A, 1, (uint8_t[]){0x55});

  // FRAME RATIO CONTROL, STANDARD RGB COLOR
  ili9341_spi_write_command_data(lcd, issNONE, 0xB1, 2,
                                 (uint8_t[]){0x00, 0x18});

  // DISPLAY FUNCTION CONTROL
  ili9341_spi_write_command_data(lcd, issNONE, 0xB6, 3,
                                 (uint8_t[]){0x08, 0x82, 0x27});

  // 3GAMMA FUNCTION DISABLE
  ili9341_spi_write_command_data(lcd, issNONE, 0xF2, 1, (uint8_t[]){0x00});

  // GAMMA CURVE SELECTED
  ili9341_spi_write_command_data(lcd, issNONE, 0x26, 1, (uint8_t[]){0x01});

  // POSITIVE GAMMA CORRECTION
  ili9341_spi_write_command_data(lcd, issNONE, 0xE0, 15,
                                 (uint8_t[]){0x0F, 0x31, 0x2B, 0x0C, 0x0E, 0x08,
                                             0x4E, 0xF1, 0x37, 0x07, 0x10, 0x03,
                                             0x0E, 0x09, 0x00});

  // NEGATIVE GAMMA CORRECTION
  ili9341_spi_write_command_data(lcd, issNONE, 0xE1, 15,
  ...
  // MADCTL
  ili9341_spi_write_command_data(
      lcd, issNONE, 0x36, 1,
      (uint8_t[]){ili9341_screen_rotation(lcd->orientation)});
...
ili9341_two_dimension_t ili9341_project_touch_coordinate(ili9341_t *lcd,
  ...
  if (NULL != lcd) {
    ...
    case itcScalar:
      ...
      x_scaled =
          coord.x; // otherwise we get "may be used uninitialized" warning even
      y_scaled = coord.y; // though all switch() cases are handled

      switch (lcd->orientation % isoCOUNT) {
      case isoPortrait:
      case isoPortraitFlip:
        x_scaled = interp(coord.x, lcd->touch_scalar.min.x,
                          lcd->touch_scalar.max.x, 0U, lcd->screen_size.width);
        y_scaled = interp(coord.y, lcd->touch_scalar.min.y,
                          lcd->touch_scalar.max.y, 0U, lcd->screen_size.height);
        break;

      case isoLandscape:
        x_scaled = interp(coord.y, lcd->touch_scalar.min.y,
                          lcd->touch_scalar.max.y, 0U, lcd->screen_size.width);
        y_scaled = interp(coord.x, lcd->touch_scalar.min.x,
                          lcd->touch_scalar.max.x, 0U, lcd->screen_size.height);
        break;
      ...
      coord = ili9341_clip_touch_coordinate(
          ...
          (ili9341_two_dimension_t){{lcd->screen_size.width},
                                    {lcd->screen_size.height}});
    ...
    case itc3Point:
      ...
      coord = ili9341_clip_touch_coordinate(
          rotate, (ili9341_two_dimension_t){{0U}, {0U}},
          lcd->touch_3point.scale);
      ...
      case isoPortrait:
        rotate = (ili9341_two_dimension_t){
            .x = lcd->touch_3point.scale.width - coord.y, .y = coord.x};
        break;

      case isoLandscape:
        rotate = (ili9341_two_dimension_t){.x = coord.x, .y = coord.y};
        break;

      case isoPortraitFlip:
        rotate = (ili9341_two_dimension_t){
            .x = coord.y, .y = lcd->touch_3point.scale.height - coord.x};
        break;
