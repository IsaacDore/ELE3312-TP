...
static ili9341_bool_t ili9341_clip_rect(ili9341_t *lcd, int16_t *x, int16_t *y,
...
void ili9341_spi_tft_set_address_rect(ili9341_t *lcd, uint16_t x0, uint16_t y0,
  ...
  ili9341_spi_write_command_data(
      lcd, issNONE, 0x2A, 4,
      (uint8_t[]){(x0 >> 8) & 0xFF, x0 & 0xFF, (x1 >> 8) & 0xFF, x1 & 0xFF});

  // row address set
  ili9341_spi_write_command_data(
      lcd, issNONE, 0x2B, 4,
      (uint8_t[]){(y0 >> 8) & 0xFF, y0 & 0xFF, (y1 >> 8) & 0xFF, y1 & 0xFF});
...
void ili9341_draw_pixel(ili9341_t *lcd, ili9341_color_t color, int16_t x,
                        int16_t y) {
...
void ili9341_draw_line(ili9341_t *lcd, ili9341_color_t color, int16_t x0,
  ...
  while (x0 <= x1) {
    ...
    if ((x >= 0) && (x <= lcd->screen_size.width) && (y >= 0) &&
        (y <= lcd->screen_size.height)) {
...
void ili9341_draw_rect(ili9341_t *lcd, ili9341_color_t color, int16_t x,
...
void ili9341_fill_rect(ili9341_t *lcd, ili9341_color_t color, int16_t x,
                       int16_t y, uint16_t w, uint16_t h) {
...
void ili9341_draw_circle(ili9341_t *lcd, ili9341_color_t color, int16_t x,
                         int16_t y, int16_t r) {
  int16_t f = 1 - r;
  int16_t fx = 1;
  int16_t fy = -2 * r;
...
void ili9341_fill_circle(ili9341_t *lcd, ili9341_color_t color, int16_t x,
                         int16_t y, int16_t r) {
...
void ili9341_fill_screen(ili9341_t *lcd, ili9341_color_t color) {
  ili9341_fill_rect(lcd, color, 0, 0, lcd->screen_size.width,
                    lcd->screen_size.height);
}

void ili9341_draw_bitmap_1b(ili9341_t *lcd, ili9341_color_t fg_color,
                            ili9341_color_t bg_color, int16_t x, int16_t y,
                            uint16_t w, uint16_t h, uint8_t *bmp) {
...
void ili9341_draw_char(ili9341_t *lcd, ili9341_text_attr_t attr, char ch) {
  ...
  // select target region
  ili9341_spi_tft_set_address_rect(lcd, attr.origin_x, attr.origin_y,
                                   attr.origin_x + attr.font->width - 1,
                                   attr.origin_y + attr.font->height - 1);
  ili9341_spi_tft_select(lcd);
